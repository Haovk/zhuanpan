$(function () {
    ...
    function clickFn() {
        ...
        var newmark=random;
        random=8-mark+random;        
        mark = newmark;
        mark %= 8;
        $('#showimage'+(newmark+1)).show();
        //默认先转4圈
        random += 32;//圈数 * 奖品总数
        //调用旋转动画
        ...
        function (inputValue){
            if(inputValue){
                ...
                if(!re.test(inputValue)){
                    ...
                    $.ajax({
                        url: "/nineturntable/bindUser",
                        method: 'post',
                        dataType: "json",
                        data: { id: $tid.val(),userid:inputValue },
                        success: function (data) {
